...
public class GenericTaskEngineTcpManagementWorker implements Runnable {
	...
	protected GenericTaskEngineTcpManagementWorker(Socket clientSocket, GenericTaskEngineEntity engine, String input, String taskDir, boolean exit) {
	...
	public void run() {
			try {
			    ...
			    if(!new File(taskDir + "/" + clientSocket.getInetAddress().getHostAddress() + "." + clientSocket.getPort(), input.split(" ")[1]).exists()) {
			    	boolean success = (new File(taskDir + "/" + clientSocket.getInetAddress().getHostAddress() + "." + clientSocket.getPort())).mkdirs();
				    if (!success) {
				    	//System.out.println("File);
				    }
	                File file = new File(taskDir + "/" + clientSocket.getInetAddress().getHostAddress() + "." + clientSocket.getPort() + "/" + input.split(" ")[1]);
				...
				if(engine.getLoad() + SchedulerHelper.getLoadForString(input.split(" ")[2]) < 100) {
					...
					String command = input.split("\"")[1].replaceAll(input.split("\"")[1].split(" ")[2], taskDir + "/" + clientSocket.getInetAddress().getHostAddress() + "." + clientSocket.getPort() + "/" + input.split("\"")[1].split(" ")[2]);
